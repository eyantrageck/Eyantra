...
const AdminLayout = () => {
  ...
  const handleLogout = () => {
    ...
    // Optional: also remove full login data if you want
          ...
          >
            <Link to="/">
              <img
                src="./image/geck.gif"
                alt="Logo"
                className="h-12 lg:h-16 w-20 object-contain"
              />
            </Link>
            <Link to="https://www.geckishanganj.org/" target="_blank">
              <img
                src="./image/e_yantra.svg"
                alt="E-Yantra Logo"
          ...
          <div className="flex items-center gap-4">
            {canRegister && (
              <Link to="/admin-register">
                <button className="bg-green-600 cursor-pointer text-white px-4 py-2 rounded-lg hover:bg-green-700 transition-all shadow-sm">
                  Register Admin
                </button>
              </Link>
            )}
